...
**规则/决策**:
- **生产凭证与开发凭证彻底分离**:schema.sql 只含 localhost Docker 开发凭证;admin/ops dev 密码(`dev-admin-2026`/`dev-ops-2026`)明文只在本地新人上手指南(不入 git);生产凭证仍只存在服务器 external.yml,部署时必须 ALTER USER 改独立强密码。换哈希只影响全新初始化的库,现有 dev/prod 库不动(账号已按台账轮换过)。
