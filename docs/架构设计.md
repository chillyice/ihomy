...
> **数字口径**:70 张表 / 39 个 Controller / service 包 58 个类 / 63 个活跃实体 / 63 个 Mapper(18 个自定义 XML),均为 2026-09-26 对照代码实测,复核方法见文末附录。
...
后端 39 个 Controller / service 包 58 个类,按业务域分组如下(域划分与 `AGENTS.md` 功能模块清单一致):
...
前端与后端域一一对应:`api/index.js` 35 个 Api 对象按模块分组,`views/` 55 个页面组件挂 50 条懒加载路由,Pinia 只有两个全局 store(user 登录态 + app 首页聚合)——**业务状态分散在各页面自行管理,不搞大 store**,页面间共享的只有登录态与首页聚合数据。
...
**为什么 2GB 生产机这样分**:MySQL 调优后本机部署最省内存;Redis 轻量且升级频繁,容器化;后端用虚拟线程(JDK 21)在小堆下也能扛 IO 密集负载。SSH 端口统一 19068(禁 22)。详细步骤在 `docs/部署指导-Linux.md` / `docs/部署指导-Windows.md`(脱敏入库版;生产凭证台账在维护者本地,不入 git)。
