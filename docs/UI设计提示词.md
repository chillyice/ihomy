...
- **侧栏家具预设**:家具 tab 8 种预设(衣柜/床/书桌/餐桌/沙发/茶几/冰箱/柜子,真实比例形状块),**拖入画布自动关联所在房间、命名 类型+序号(同类型递增)、落点居中**;房间列表(复制/删除按钮);库 tab 拖回房间。
...
42. **物品定位户型图**(`/item` 页面主视图,非 tab,全宽 SVG,查看态手绘草图风滤镜/编辑态直线直角):顶栏=房子下拉→搜索框→「列表」「编辑」;两模式分离(查看默认+编辑点「编辑」进入);编辑态左侧酷家乐式侧栏(房间 tab=画图工具+房间列表[复制/删除]/家具 tab=8 种预设拖入+列表/库 tab=拖回),侧栏出现画布 ResizeObserver 自动 fit;左下角楼层切换器(编辑态常显+「+」加层,空层不消失;查看态 ≥2 层显示);空态「+」建房子、空楼层引导进编辑;编辑手势=拖主体平移(家具级联)/拖直角顶点角缩放(Ctrl=自由拖斜边)/拖边拉平+单轴锁定+邻边吸直/家具 8 向缩放/拖物品定位/空白+滚轮+双指 pinch 平移缩放;画房间=绘制矩形+逐点描绘(双击闭合,十字光标);房间裁剪=剪刀图标+合法性虚线(绿/暗红)拆两房间(序号递进,家具按中心分配);房间粘合=牙膏筒图标+共享边虚线合并(命名 A-B);双击名称改名、右键删除;边长常显+双击边手柄数值输入+房间面积 m²;底图支持图片/PDF(pdfjs 转 PNG)+不透明度滑杆(按楼层)+标定两点测距(带 tooltip);吸附=顶点磁吸 6px+家具贴墙+边多层对齐(绿虚线);撤销 Ctrl+Z(栈 50);重叠警示红描边(共边不误报);搜索命中/点家具物品浮现,多结果右下角列表切房/楼层+高亮;移动端只读+pinch 缩放无画布编辑。
