...
> 设计已冻结,详见 `docs/户型图设计.md`(v4)。以下为 UI 规格。
...
- **房间**:正交多边形,描边暖棕 `rgba(184,140,110,0.6)` 2px、填充 `rgba(184,140,110,0.14)`、圆角 8px;**命名预设(客厅/主卧/次卧/厨房/卫生间/书房…)+自定义**;房间名左上(名字 + 真实尺寸 `4.2×3.5m` + 面积 `xx.xx m²`);右上 hover 出编辑/删除图标;顶点手柄=白底暖棕圆点,边中点手柄=小横杠。
- **编辑手势**:拖主体=全顶点平移;拖顶点=改单点;拖边中点=边平移(相邻两点跟随,保持直角);双击边=插顶点(造缺口);双击顶点=删顶点(去缺口,最少 4 点);拖家具主体=移动、拖家具边/角=缩放;拖物品标记=定位;空白拖拽=平移画布、滚轮=缩放。
- **画房间两种方式**:① 拖拽拉矩形;② 点击逐点描绘(点顶点→点下一个→…→双击闭合,直接画 L 形/缺口)。
- **搜索定位**:物品标记**不常显**,搜索命中或点家具才浮现;多结果右下角弹结果列表(跨房子/楼层/位置分组,当前层命中显著),点某条 → 自动切房/楼层 + 画布高亮(暖棕光环 + 放大镜脉冲)。
- **吸附**:边对边/顶点对顶点阈值 6px,吸附时显示对齐虚线(暖棕 dashed)。
