...
> S1(数据模型)+ S2(页面骨架+编辑手势)已实现;设计详见 `docs/户型图设计.md`(v4 + 文末「实现记录」)。以下为实装 UI 规格。

- **页面结构 = 户型图主视图(非 tab)**:顶栏常驻 房子下拉 → 搜索框(搜物品名 → 画布高亮定位)→「列表」「编辑」;主体整块是画布,不加独立导航。**楼层切换器在左下角**(竖排 `1F/2F/3F`,商厦电梯模式,没画图的房子不显示、默认 1F,含「+加楼层」)。
- **两模式分离**:查看/找东西(默认)与 编辑 分开;点「编辑」进入编辑态,左侧出**酷家乐式侧栏(房间/家具/库子 tab,拖拽素材入画布)**,点「完成」退出;查看态隐藏侧栏、不误拖。
- **三态**:空态(无房子,画布中央大「+」→ 建房子)/ 查看态(直接看户型图 + 搜索定位)/ 编辑态(画房间/摆家具/放物品/上传底图/标定)。
- **加数据两方式并存**:「列表」= 旧 CRUD(房子/房间/家具/物品,不画图也能用);画布内 = 画完/摆完弹窗命名,一步建实体+几何。
- **渲染**:SVG 单层 `<svg>`,手绘草图风(铅笔线条感,契合暖棕毛玻璃主题);房间 `<polygon>`、家具 `<rect>`、物品 `<g>`、底图 `<image>` 铺原点;画布外层 `transform: translate() scale()` 实现平移缩放。
- **徒手画一等公民**:底图可选;无图也画得舒服,不是人人有户型图。
- **房间**:多边形(**允许斜边**——去掉强制直角),描边暖棕 `rgba(184,140,110,0.6)` 2px、填充 `rgba(184,140,110,0.14)`;命名预设(客厅/主卧/次卧/厨房/卫生间/书房…)+自定义;顶点手柄=白底暖棕圆点,边中点手柄=小方块。
- **编辑手势**:拖主体=全顶点平移;拖顶点=自由改单点(允许斜边,当邻边贴近横/纵轴 6px 内**轴对齐吸附**成矩形、吸附瞬间手柄变绿 `#6b9b6b`);拖边=两端点一起自由平移;hover 边(12px 内)=**圆圈加号**随鼠标沿边投影点移动,点击插入端点(边 ab→ad+db,鼠标靠近顶点/边中点手柄 12px 内不显示加号);双击顶点=删顶点(最少 4 点);拖家具主体=移动、拖右下角=缩放;拖物品=定位;空白拖拽=平移画布、滚轮=缩放。
- **画房间两种方式**:① 拖拽拉矩形;② 点击逐点描绘(点顶点→点下一个→…→双击闭合,直接画 L 形/缺口)。
- **搜索定位**:物品标记**不常显**,搜索命中或点家具才浮现;多结果右下角弹结果列表(跨房子/楼层/位置分组,当前层命中显著),点某条 → 自动切房/楼层 + 画布高亮。
- **吸附(轴对齐)**:拖顶点时邻边贴近横/纵轴阈值 6px 吸附对齐(辅助成矩形);边对边/顶点对顶点磁吸 + 对齐虚线留待 S3。
- **尺寸标注**:默认 `100px/m` 即显示米数;边长常显开关(开=所有边常显米数);关闭时常显「受影响的边」——拖角显示角的两条边、拖边显示相邻两条边(拖边本身长度不变);选中边/角弹数值输入框键入精确米数(3 位小数,拖拽定型 + 数值雕刻);标定后更准(非「未标定不显示」)。
- **底图标定(可选校正)**:点「标定」进入模式,点击底图两点(虚线连线 + 圆点标记),输入实际米数 → 校正当前楼层 `scale`(默认 100px/m)。
- **家具**:矩形,带 `type`(预设衣柜/床/冰箱/书桌/沙发/茶几/柜子/餐桌/书架/其他 + 可自定义,配图标)+ 自定义 `name`(区分同种家具,如「床1」);`room_id` 空=家具库(删房间进库、绑定不动,从侧栏「库」拖回);随房间平移(级联)、不随房间变形、不缩放(靠墙橱柜墙动柜不动);嵌入式柜子矩形允许跨出房间边界;家具在画布内摆好后弹窗命名。
- **物品**:锚家具/房间,`rel_x/rel_y` 相对包围盒 0~1,`position` 文本承载精确描述(如「第三层左侧抽屉」,无抽屉层级);已放置物品=圆形头像图+名称标签(居中于坐标点);未放置物品=虚线 chip(名称)收在房间左上角,拖出即放置。
- **未放置房间**(仅 CRUD 列表创建、尚未画几何的房间):右侧栏 `section-label` 列表,点击放到画布默认位置。
- **深色模式**:viewport `#1E2A48`、边框 `rgba(232,220,200,0.15)`;房间描边 `rgba(212,178,152,0.5)`、填充 `rgba(212,178,152,0.15)`、文字 `#E8DCC8`。
- **移动端**:只读户型图 + 搜索定位;可增删改房子/房间/家具/物品(列表 CRUD);**不做画布编辑**(编辑户型图桌面专属)。
...
42. **物品定位户型图**(`/item` 页面主视图,非 tab,SVG 手绘草图风渲染):顶栏=房子下拉→搜索框→「列表」「编辑」;两模式分离(查看/找东西默认 + 编辑点「编辑」进入、点「完成」退出);编辑态左侧酷家乐式侧栏(房间/家具/库子 tab,拖拽素材入画布),查看态隐藏;空态画布中央大「+」加房子,编辑完每次进来直接看户型图;左下角楼层切换器(竖排 1F/2F/3F,商厦电梯模式,没画图的房子不显示默认 1F,含+加楼层);房间正交多边形(描边暖棕+填充半透明+顶点/边中点手柄,命名预设+自定义),画房间支持拖矩形 + 点击逐点描绘(双击闭合),缺口/L 形通过双击边加顶点、双击顶点删点实现;房/房间/家具/物品在画布内创建(命名弹窗),「列表」=旧 CRUD;底图/标定按楼层,默认 100px/m、标定=可选校正;拖主体平移、拖顶点改角、拖边中点平移边、拖家具移动/缩放、拖物品定位;家具带 type(预设+自定义图标)+自定义 name,`room_id` 空=家具库(删房间进库、绑定不动),随房间整体平移、不随房间变形(靠墙柜墙动柜不动);吸附边对边/顶点对顶点阈值 6px + 对齐虚线;边长常显开关(关闭时拖角显示角两边、拖边显示相邻两边)+ 选中边/角数值输入精确米数(毫米);房间名旁显示真实尺寸 + 面积(m²);物品标记不常显、搜索/点家具才浮现,多结果右下角弹结果列表(跨房子/楼层/位置分组,当前层命中显著)点某条自动切房/楼层+画布高亮;未放置房间右侧栏点击放置、未放置物品虚线 chip 收在房间左上角;移动端只读户型图+列表 CRUD,无画布编辑。
