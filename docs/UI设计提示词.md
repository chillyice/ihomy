...
45. **设置页·家庭 AI 配置面板**(`Settings` 分类「AI 配置」🤖,2026-09-08 V9.43,仅家长可见——菜单项 `v-if userStore.hasPerm('family:manage')`,与其他设置大类同款侧栏导航):单张 `.card.settings-card`+`.section-label` 标题「AI API 配置(当前家庭)」+**能力状态徽标三枚**(`.ai-cap-row` flex-wrap,chat/image/asr,可用=主题色描边+浅色底/未配置=灰描边灰字,12px 圆角胶囊)+说明文案 `.share-tip`(留空跟随全局/模型名才启用/Key 加密不回传)+el-form label-position=top:**服务地址/对话模型/超时 el-input-number 1000~600000 step 1000**+图片模型/图片服务地址/图片 Key+语音模型/语音服务地址/语音 Key+备注 el-input maxlength 200;**未配置字段占位符显示当前全局兜底值**(「留空跟随全局(当前:GLM-5.3-Flash)」,由 GET /ai/config 的 global 段驱动);**密钥输入框 type=password show-password**,占位「已配置,留空保留原值」(已配置)/「留空跟随全局兜底」(未配置),保存后清空输入;表单分区 el-divider(主配置/图片/语音);底部 `.form-footer` 右对齐=保存 AI 配置(primary)+恢复默认(ghost,仅 configured 时显示,ElMessageBox 确认「删除后该家庭 AI 调用将跟随全局兜底配置」);v-loading 加载。
