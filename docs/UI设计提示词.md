...
52. **壁纸氛围屏**(`/wallpaper`,V9.87,独立全屏——`meta.standalone` 不套 ihomy 外壳,无侧栏/页脚/回顶/播放器;不引 Element Plus,全原生控件 + 主题 CSS 变量):
   ...
   - **角落控件(右下;≤768px 改右上)**:`.wp-hint`(12px 次要色,`@media (hover:none)` 隐藏)→ 两组 `.wp-seg` 分段器(半透明卡片底 `rgba(var(--color-card-rgb),.74)` + 圆角 12px):主题(暖居/光尘)+ 晨暮(自动/晨/暮);`.wp-segbtn` `all:unset` + 内边距 6px 12px + 圆角 9px,选中 `.on` 品牌色底 + `--color-brand-text`;整块 opacity/translateY .5s 过渡,`pointer-events` 随显隐切换(隐藏时不吃点击)。
