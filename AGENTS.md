...
- **运维管理员(已实施,V3.8)**:账号 `ops / ops@ihomy.local` 密码 ***REMOVED-INIT-PASSWORD***(OPS 角色,仅 ops/auth 白名单);`OpsAccessFilter`:OPS 角色只放行 `/api/ops/**` 与 `/api/auth/**`(访问其它业务接口 403),非 OPS 访问 `/api/ops/**` 一律 403;router.beforeEach 对 OPS 非 /ops 页面重定向 /ops;App.vue 对 OPS 隐藏 SideTabs/MusicPlayer,AppHeader 运维态隐藏模块导航/通知铃铛,品牌显示"运维管理"点击回 /ops;`views/ops/Ops.vue` 仅系统级信息(内存/磁盘/接口健康/操作日志/异常统计),不返回任何用户隐私数据。
