...
- **OWNER 账号每页都弹一次「无权限访问」**(2026-09-24 归档 V9.89 时实测发现,既有缺陷,与 V9.89 无关)：OWNER 的 `permissions` 数组里带着 `ops:view` 而 `isOps=false`(实测 `admin@ihomy.local`:role=OWNER、isOps=false、perms 含 `ops:view`),侧边栏 `AppSidebar.vue` 的 `loadOssUpdateCount` 用 `userStore.hasPerm('ops:view')` 判断就去拉 `/api/ops/oss/summary`,后端 `OpsAccessFilter` 却按 **OPS 角色绑定**放行 → 403,右上角弹「无权限访问」(带 tid)。影响面:所有非 OPS 但权限数组含 `ops:view` 的账号(OWNER 全员)。**修法**:前端改用 `userStore.isOps` 判断(或后端让权限数组不含 `ops:view`);**顺带**:这是「前端权限码」与「OPS 隔离」两套口径不一致,改动时确认 OPS 侧只走 `isOps`。
...
- 前端构建:`cd frontend; npm run build` → 入口 chunk ≈318.7KB(2026-09-24 V9.89 实测 318.71KB/gzip 127.79KB,较 V9.87 的 317.39KB 增 1.32KB=壁纸令牌双语说明文案;旧基线 2026-09-08 V9.48 为 246.35KB/97.70KB;pdfjs 已隔离为独立异步 chunk ~483KB 仅 PDF 场景加载;simple-mind-map ~341KB 仅脑图编辑页加载;壁纸页独立 chunk 7.70KB/4.78KB 懒加载)
