...
--   本地开发：.\scripts\start-db.ps1（Docker 首次启动空库时自动导入）
--   手动执行：mysql -uroot -p < schema.sql
--
-- 说明:
...
--   3. 本文件为开发安全版（入 git）：ihomy 账号密码是本机开发固定值，仅绑定 localhost
--      Docker 环境；生产部署时必须 ALTER USER 改为独立强密码（写入服务器上的
--      external.yml，不入仓库），详见 Linux部署指导.md。
--   4. admin/ops 初始密码为开发专用值（明文记于本地 docs/新人上手指南.md，不入 git），
--      与生产凭证不同；生产部署后应立即修改。
-- ============================================================
...
-- ------------------------------------------------------------
CREATE USER IF NOT EXISTS 'ihomy'@'localhost' IDENTIFIED BY 'ihomy-dev-2026!';
CREATE USER IF NOT EXISTS 'ihomy'@'%'        IDENTIFIED BY 'ihomy-dev-2026!';
ALTER  USER 'ihomy'@'localhost' IDENTIFIED BY 'ihomy-dev-2026!';
ALTER  USER 'ihomy'@'%'        IDENTIFIED BY 'ihomy-dev-2026!';
...
-- ------------------------------------------------------------
-- 16a. content_blog_category 博客分类表
-- ------------------------------------------------------------
...
CREATE TABLE `content_photo_album` (
  ...
  `source_device_id` BIGINT     DEFAULT NULL COMMENT '映射来源设备ID(非空=映射相册)',
  `source_path`    VARCHAR(500) DEFAULT NULL COMMENT '设备上的远程目录路径',
  `sync_status`    VARCHAR(20)  DEFAULT NULL COMMENT '映射状态:VALID可访问/OFFLINE设备离线/MISSING目录丢失',
  `last_synced_at` DATETIME     DEFAULT NULL COMMENT '最后同步刷新时间',
...
CREATE TABLE `content_video` (
  ...
  `video_url`      VARCHAR(500) NOT NULL COMMENT '视频文件URL（本地上传/files/ 或设备映射 storage:// 逻辑地址）',
  `uploader_id`    BIGINT       NOT NULL COMMENT '上传者ID',
  `family_id`      BIGINT       NOT NULL COMMENT '家庭ID',
  `visibility` VARCHAR(20) NOT NULL DEFAULT 'FAMILY' COMMENT '可见范围：PRIVATE仅自己/FAMILY家庭可见/PUBLIC公开',
  `source_device_id` BIGINT       DEFAULT NULL COMMENT '来源设备ID（设备目录映射）',
  `source_path`    VARCHAR(500) DEFAULT NULL COMMENT '设备文件去重键 dev:{deviceId}:{path}',
  `source_fs_id`   BIGINT       DEFAULT NULL COMMENT '百度网盘 fs_id',
  `source_dir`     VARCHAR(500) DEFAULT NULL COMMENT '映射的根目录（相对设备，展示用）',
...
-- ============================================================
-- 初始化完成。
-- 应用连接请使用账号 ihomy（密码为你执行前替换的强密码，存于 external.yml），不要用 root。
-- 验证账号权限：
...
CREATE TABLE `family_house` (
  ...
  `floor_plans` TEXT         DEFAULT NULL COMMENT '楼层户型图配置(JSON,key=楼层号:{imageUrl,scale},默认scale=100px/m)',
  `sort_order`  INT          NOT NULL DEFAULT 0 COMMENT '排序',
  `created_by`  BIGINT       DEFAULT NULL COMMENT '创建人ID',
  `created_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `family_room` (
  ...
  `geometry`    TEXT         DEFAULT NULL COMMENT '房间正交多边形几何(JSON顶点数组:[{x,y}...],空=未画户型图)',
  `created_by`  BIGINT       DEFAULT NULL COMMENT '创建人ID',
  `created_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `family_furniture` (
  `id`          BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
  `family_id`   BIGINT       NOT NULL COMMENT '所属家庭ID',
  `room_id`     BIGINT       DEFAULT NULL COMMENT '所在房间ID(可空,空=家具库)',
  `name`        VARCHAR(50)  NOT NULL COMMENT '家具名(如 柜子/茶几/冰箱)',
  `type`        VARCHAR(20)  DEFAULT NULL COMMENT '家具类型(衣柜/床/冰箱/书桌/沙发/茶几/柜子/餐桌/书架/其他,可自定义)',
  `x`           DECIMAL(12,2) DEFAULT NULL COMMENT '画布X坐标(px,空=未摆放)',
  `y`           DECIMAL(12,2) DEFAULT NULL COMMENT '画布Y坐标(px,空=未摆放)',
  `w`           DECIMAL(12,2) DEFAULT NULL COMMENT '画布宽(px,空=未摆放)',
  `h`           DECIMAL(12,2) DEFAULT NULL COMMENT '画布高(px,空=未摆放)',
  `note`        VARCHAR(255) DEFAULT NULL COMMENT '备注',
  `created_by`  BIGINT       DEFAULT NULL COMMENT '创建人ID',
  `created_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `family_item` (
  `id`            BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
  `family_id`     BIGINT       NOT NULL COMMENT '所属家庭ID',
  `furniture_id`  BIGINT       DEFAULT NULL COMMENT '所在家具ID(可空)',
  `room_id`       BIGINT       DEFAULT NULL COMMENT '所在房间ID(可空,散放物品锚房间)',
  ...
  `stored_at`       DATETIME     DEFAULT NULL COMMENT '存入时间(食材,默认录入时)',
  `shelf_life`      INT          DEFAULT NULL COMMENT '保质期数值(食材)',
  `shelf_life_unit` VARCHAR(10)  DEFAULT NULL COMMENT '保质期单位:HOUR小时/DAY天/MONTH月(食材,录入默认天)',
  `rel_x`         DECIMAL(5,4) DEFAULT NULL COMMENT '相对锚点包围盒X(0~1,锚家具或房间)',
  `rel_y`         DECIMAL(5,4) DEFAULT NULL COMMENT '相对锚点包围盒Y(0~1,锚家具或房间)',
  `created_by`    BIGINT       DEFAULT NULL COMMENT '创建人ID',
  `created_at`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
-- ------------------------------------------------------------
-- 脑图(工具箱-脑图设计):家庭级共享,data 存前端 simple-mind-map 的完整 JSON
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `content_mindmap`;
CREATE TABLE `content_mindmap` (
  `id`         BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
  `family_id`  BIGINT       NOT NULL COMMENT '所属家庭ID',
  `user_id`    BIGINT       NOT NULL COMMENT '创建人ID',
  `title`      VARCHAR(100) NOT NULL COMMENT '脑图标题',
  `data`       LONGTEXT     DEFAULT NULL COMMENT '脑图数据JSON(layout/root/theme/view/config)',
  `thumb_url`  MEDIUMTEXT   DEFAULT NULL COMMENT '列表缩略图(data URL,编辑器保存时生成)',
  `deleted`    TINYINT      NOT NULL DEFAULT 0 COMMENT '逻辑删除',
  `created_at` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
-- ------------------------------------------------------------
-- 脑图历史版本快照:编辑器定期/手动存档,误删可回滚(每图保留最近 20 份,超出自动清理)
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `content_mindmap_snapshot`;
CREATE TABLE `content_mindmap_snapshot` (
  `id`         BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
